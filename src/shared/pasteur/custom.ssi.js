...
var slide_ssi = (function($) {
  ...
  var onSlideLoad = function() {
    ...
    var SSI_section = localStorage.getItem(LOCALSTORAGE_SSI_VARIABLE);

    // If we are on a slide that has no safety information.
    ...
    if (!SSI_section || SSI_section == "NaN") {
      displaySSISection($('.slide-ssi-copy-1'));
    ...
    localStorage.setItem(LOCALSTORAGE_SSI_VARIABLE, next_ssi_section_num);

    // Show the next SSI section. Remember this is an array so minus 1 for index number.
